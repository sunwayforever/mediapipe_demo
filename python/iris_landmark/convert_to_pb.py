...
output_savedmodel_path = "../../model/iris_landmark"
model_json_path = "../../model/iris_landmark.json"
num_tensors = 386
...
def make_graph(ops, op_types, interpreter):
    ...
    for index, op in enumerate(ops):
        ...
        if op_type == "CONV_2D":
            ...
            paddings = tf.constant(paddings_array, name=paddings_detail["name"])
            output_tensor = tf.pad(input_tensor, paddings, name=output_detail["name"])
            tensors[output_detail["index"]] = output_tensor
        elif op_type == "RELU":
            output_detail = interpreter._get_tensor_details(op["outputs"][0])
            input_tensor = tensors[op["inputs"][0]]
...
def main():
    ...
    with tf.compat.v1.Session(config=config, graph=graph) as sess:
        ...
        tf.compat.v1.saved_model.simple_save(
            ...
            outputs={
                "output_eyes_contours_and_brows": graph.get_tensor_by_name(
                    "output_eyes_contours_and_brows:0"
                ),
