...
class spin_wait {
   public:
    ...
    unsigned int count() const { return m_k; }
...
// Pause tries if the processor supports it
   ...
   private:
    ...
    bool yield_or_sleep() {
        if (!m_ul_yield_only_counts) {  // If yield-only limit was reached then
                                        // yield one in every NUM_OF_YIELDS
                                        // tries
            return (m_k % NUM_OF_YIELDS) != 0;
        } else {  // Try to see if we've reached yield-only time limit
            const ipcdetail::OS_highres_count_t now =
                ipcdetail::get_current_system_highres_count();
            const ipcdetail::OS_highres_count_t elapsed =
                ipcdetail::system_highres_count_subtract(now, m_count_start);
            if (!ipcdetail::system_highres_count_less_ul(
                    elapsed, m_ul_yield_only_counts)) {
#ifdef BOOST_INTERPROCESS_SPIN_WAIT_DEBUG
                std::cout << "elapsed!\n"
                          << "  m_ul_yield_only_counts: "
                          << m_ul_yield_only_counts << " system tick(us): "
                          << ipcdetail::get_system_tick_us() << '\n'
                          << "  m_k: " << m_k << " elapsed counts: ";
                ipcdetail::ostream_highres_count(std::cout, elapsed)
                    << std::endl;
#endif
