...
class condition_algorithm_8a {
   ...
   public:
    template <class Lock>
    static bool wait(ConditionMembers &data, Lock &lock, bool timeout_enabled,
                     const boost::posix_time::ptime &abs_time);
    static void signal(ConditionMembers &data, bool broadcast);
};

template <class ConditionMembers>
inline void condition_algorithm_8a<ConditionMembers>::signal(
    ConditionMembers &data, bool broadcast) {
...

    {
        ...
        if (0 != data.get_nwaiters_to_unblock()) {   // the gate is closed!!!
            if (0 == data.get_nwaiters_blocked()) {  // NO-OP
                // locker's destructor triggers
                // data.get_mtx_unblock_lock().unlock()
                return;
            }
            if (broadcast) {
                data.get_nwaiters_to_unblock() += nsignals_to_issue =
        ...
        } else if (data.get_nwaiters_blocked() >
                   data.get_nwaiters_gone()) {  // HARMLESS RACE CONDITION!
            ...
            if (broadcast) {
                nsignals_to_issue = data.get_nwaiters_to_unblock() =
                    data.get_nwaiters_blocked();
                data.get_nwaiters_blocked() = 0;
            } else {
...
inline bool condition_algorithm_8a<ConditionMembers>::wait(
    ConditionMembers &data, Lock &lock, bool tout_enabled,
    const boost::posix_time::ptime &abs_time) {
    ...
    bTimedOut = tout_enabled ? !data.get_sem_block_queue().timed_wait(abs_time)
            ...
            if (0 == --data.get_nwaiters_to_unblock()) {
                if (0 != data.get_nwaiters_blocked()) {
                    ...
                    nsignals_was_left = 0;  // do not open the gate below again.
                           ...
                           (nwaiters_was_gone = data.get_nwaiters_gone())) {
                    data.get_nwaiters_gone() = 0;
                }
            }
        } else if ((std::numeric_limits<integer_type>::max)() / 2 ==
                   ++data.get_nwaiters_gone()) {  // timeout/canceled or
                                                  // spurious semaphore :-)
            data.get_sem_block_lock().wait();
            data.get_nwaiters_blocked() -=
                data.get_nwaiters_gone();  // something is going on here - test
                                           // of timeouts? :-)
            data.get_sem_block_lock().post();
            data.get_nwaiters_gone() = 0;
        }
    ...
    if (1 == nsignals_was_left) {
        if (0 != nwaiters_was_gone) {
            ...
            while (nwaiters_was_gone--) {
                data.get_sem_block_queue()
                    .wait();  // better now than spurious later
            }
        }
        data.get_sem_block_lock().post();  // open the gate
    }
...
class condition_8a_wrapper {
   ...
   public:
    ...
    ConditionMembers &get_members() { return m_data; }

    const ConditionMembers &get_members() const { return m_data; }

    void notify_one() { algo_type::signal(m_data, false); }

    void notify_all() { algo_type::signal(m_data, true); }
    ...
    bool timed_wait(L &lock, const boost::posix_time::ptime &abs_time,
        ...
        if (!lock) throw lock_exception();
        while (!pred()) {
            if (!algo_type::wait(m_data, lock, true, abs_time)) return pred();
        }
        return true;
    }
};
