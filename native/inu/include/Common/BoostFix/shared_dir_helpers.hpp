...
#include <string>
    ...
    defined(BOOST_INTERPROCESS_WINDOWS)
...
struct windows_bootstamp {
    ...
    // modules/dlls are from this process so internal raw pointers to heap are
    // always valid
    std::string stamp;
};

inline void get_bootstamp(std::string &s, bool add = false) {
    const windows_bootstamp &bootstamp =
        windows_intermodule_singleton<windows_bootstamp>::get();
    if (add) {
        s += bootstamp.stamp;
    } else {
        s = bootstamp.stamp;
...
inline void get_bootstamp(std::string &s, bool add = false) {
    ...
    if (::sysctl(request, 2, &result, &result_len, 0, 0) < 0) return;
    ...
    boost::uint32_t fields[2] = {boost::uint32_t(result.tv_sec),
                                 boost::uint32_t(result.tv_usec)};
...
#else
#error \
    "BOOST_INTERPROCESS_HAS_KERNEL_BOOTTIME defined with no known implementation"
#endif
...
inline void create_shared_dir_and_clean_old(std::string &shared_dir) {
    ...
    // This code caused to deletion of MQ files of our clients that use boost as
    // well (IG-2232). Since we clean old MQ files by the method
    // CleanMemoryMappedFiles and it looks like it invoked only for MQ then we
    // can eliminate this piece of code.
    /*
...
inline void create_shared_dir_cleaning_old_and_get_filepath(
    const char *filename, std::string &shared_dir) {
