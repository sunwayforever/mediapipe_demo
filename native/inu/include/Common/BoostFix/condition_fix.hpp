...
class spin_condition {
   ...
   public:
    ...
    bool timed_wait(L &lock, const boost::posix_time::ptime &abs_time) {
        if (!lock) throw lock_exception();
        // Handle infinity absolute time here to avoid complications in
        // do_timed_wait
    ...
    bool timed_wait(L &lock, const boost::posix_time::ptime &abs_time,
        ...
        while (!pred()) {
            if (!this->do_timed_wait(abs_time, *lock.mutex())) return pred();
        }
        return true;
    }
   ...
   private:
    template <class InterprocessMutex>
    bool do_timed_wait(bool tout_enabled,
    ...
    bool notify(boost::uint32_t command,
...
inline spin_condition::~spin_condition() {
    ...
    this->notify_all(
        boost::posix_time::pos_infin);  // Inuitive add timed notification
}

inline bool spin_condition::notify_one(
    const boost::posix_time::ptime
        &abs_time)  // Inuitive add timed notification
{
...
inline bool spin_condition::notify_all(
    const boost::posix_time::ptime
        &abs_time)  // Inuitive add timed notification
{
...
inline bool spin_condition::notify(
        ...
        &abs_time)  // Inuitive add timed notification
{
    ...
    spin_wait swait;
    while (SLEEP != atomic_cas32(const_cast<boost::uint32_t *>(&m_command),
                                 command, SLEEP)) {
        swait.yield();
    }
...
inline bool spin_condition::do_timed_wait(
    ...
    while (1) {
        // The thread sleeps/spins until a spin_condition commands a
        // notification Notification occurred, we will lock the checking mutex
        // so that
        spin_wait swait;
        while (atomic_read32(&m_command) == SLEEP) {
            ...
            if (tout_enabled) {
                ...
                if (now >= abs_time) {
                    ...
                    // If locking fails, indicates that another thread is
                    // executing notification, so we play the notification game
                    if (!timed_out) {
                        // There is an ongoing notification, we will try again
                        // later
                        continue;
                    }
                    // No notification in execution, since enter mutex is
                    // locked. We will execute time-out logic, so we will
                    // decrement count, release the enter mutex and return
                    // false.
                    break;
                }
            }
        ...
        if (tout_enabled && timed_out) {
            ...
            if (result == SLEEP) {
                ...
                // exit. This thread has atomically marked command as sleep
                // before so no other thread will exit. Decrement wait count.
                unlock_enter_mut = true;
                atomic_dec32(const_cast<boost::uint32_t *>(&m_num_waiters));
                break;
            } else {
                ...
                if (unlock_enter_mut) {
                    atomic_cas32(const_cast<boost::uint32_t *>(&m_command),
                                 SLEEP, NOTIFY_ALL);
                }
                break;
            }
    ...
    bool lockAcquired = mut.timed_lock(
