...
// Interprocess communication uses this intermodule singleton for whatever.
// (related bug report: https://svn.boost.org/trac/boost/ticket/9767)
//
// Changed the names to include "InuDev", this promises our own map, so we don't
// clash with different boost versions in the same process
//
//
//////////////////////////////////////////////////////////////////////////////
...
//
// 2)  A semaphore that will act as a global counter for modules attached to the
// global map
//    so that the global map can be destroyed when the last module is detached.
//
// 3)  A semaphore that will be hacked to hold the address of a heap-allocated
// map in the
...
class windows_semaphore_based_map {
   ...
   public:
    windows_semaphore_based_map() {
        ...
        //
        // Windows semaphores use 'long' parameters (32 bits in LLP64 data
        // model) and those values can't be negative, so we have 31 bits to
        // store something in max_count and initial count parameters. Also, max
        // count must be bigger than 0 and bigger or equal than initial count.
        if (sizeof(void *) == sizeof(boost::uint32_t)) {
            // This means that for 32 bit processes, a semaphore count (31
            // usable bits) is enough to store 4 byte aligned memory (4GB -> 32
            // bits - 2 bits = 30 bits). The max count will hold the pointer
            // value and current semaphore count will be zero.
            //
            // Relying in UB with a cast through union, but all known windows
            // compilers accept this (C11 also accepts this).
            union caster_union {
                void *addr;
            ...
            // Relying in UB with a cast through union, but all known windows
            // compilers accept this (C11 accepts this).
            union caster_union {
                void *addr;
                boost::uint64_t addr_uint64;
            } caster;
            caster.addr = m;
            // We'll encode the address using 30 bits in each 32 bit high and
            // low parts. High part will be the sem max count, low part will be
            // the sem initial count. (restrictions: max count > 0, initial
            // count >= 0 and max
            // count >= initial count):
            //
            // - Low part will be shifted two times (4 byte alignment) so that
            // top
            //   two bits are cleared (the top one for sign, the next one to
            //   assure low part value is always less than the high part value.
            // - The top bit of the high part will be cleared and the next bit
            // will be
            // 1
            //   (so high part is always bigger than low part due to the
            //   quasi-top bit).
            //
            //   This means that the addresses we can store must be 4 byte
            //   aligned and less than 1 ExbiBytes ( 2^60 bytes, ~1 ExaByte).
            //   User-level address space in Windows 64 is much less than this
            //   (8TB, 2^43 bytes): "1 EByte (or it was 640K?) ought to be
            //   enough for anybody" ;-).
            caster.addr = m;
        ...
        {
            success = success &&
                      m_sem_count.open_or_create(
                          name.c_str(), static_cast<long>(0),
                          winapi_semaphore_wrapper::MaxCount, perm, created);
            name = "bipc_gmap_sem_map_InuDev_";
            name += pid_creation_time;
            success =
                success && m_sem_map.open_or_create(name.c_str(), initial_count,
                                                    max_count, perm, created);
            if (!success) {
                delete m;
    ...
    ref_count_ptr *insert(const char *name, const ref_count_ptr &ref) {
        ...
        map_type::iterator it =
            map.insert(map_type::value_type(std::string(name), ref)).first;
        return &it->second;
    }
    ...
    ~windows_semaphore_based_map() {
        ...
        m_sem_map.close();
        m_sem_count.close();
        // Once scoped_lock unlocks the mutex, the destructor will close the
        // handle...
    }
...
struct thread_safe_global_map_dependant<windows_semaphore_based_map> {
    ...
    static bool remove_old_gmem() { return true; }
    ...
    static ref_count_ptr *find(windows_semaphore_based_map &map,
                               const char *name) {
        return map.find(name);
    }

    static ref_count_ptr *insert(windows_semaphore_based_map &map,
                                 const char *name, const ref_count_ptr &ref) {
