...
class message_queue_t {
   ...
   public:
    ...
    typedef typename boost::intrusive::pointer_traits<
        void_pointer>::template rebind_pointer<char>::type char_ptr;
    typedef typename boost::intrusive::pointer_traits<char_ptr>::difference_type
        difference_type;
    typedef typename boost::container::container_detail::make_unsigned<
        difference_type>::type size_type;

    //! Creates a process shared message queue with name "name". For this
    //! message queue, the maximum number of messages will be "max_num_msg" and
    //! the maximum message size will be "max_msg_size". Throws on error and if
    //! the queue was previously created.
    ...
    //! If the queue is created, the maximum number of messages will be
    //! "max_num_msg" and the maximum message size will be "max_msg_size". If
    //! queue was previously created the queue will be opened and "max_num_msg"
    //! and "max_msg_size" parameters are ignored. Throws on error.
    ...
    //! "name". If the queue was not previously created or there are no free
    //! resources, throws an error.
    ...
    //! the open constructor overload. To erase the message queue from the
    //! system use remove().
    ~message_queue_t();
    ...
    //! Sends a message stored in buffer "buffer" with size "buffer_size"
    //! through the message queue with priority "priority". If the message queue
    //! is full the sender is not blocked and returns false, otherwise returns
    //! true. Throws interprocess_error on error.
    ...
    //! the message has been successfully sent. Returns false if timeout is
    //! reached. Throws interprocess_error on error.
   ...
   private:
    ...
    bool do_receive(block_t block, void *buffer, size_type buffer_size,
                    size_type &recvd_size, unsigned int &priority,
                    const ptime &abs_time);

    bool do_send(block_t block, const void *buffer, size_type buffer_size,
                 unsigned int priority, const ptime &abs_time);

    //! Returns the needed memory size for the shared message queue.
    //! Never throws
    static size_type get_mem_size(size_type max_msg_size,
                                  size_type max_num_msg);
    typedef ipcdetail::managed_open_or_create_impl<shared_memory_object, 0,
                                                   true, false>
        open_create_impl_t;
...
class priority_functor {
    typedef typename boost::intrusive::pointer_traits<VoidPointer>::
        template rebind_pointer<msg_hdr_t<VoidPointer>>::type msg_hdr_ptr_t;
...
//!   "cur_num_msg" in a circular way, pointers point to inserted messages and
//!   the rest point to free messages. Those "cur_num_msg" pointers are ordered
//!   by the priority of the pointed message and by insertion order if two
//!   messages have the same priority. So the next message to be used in a
//!   "receive" is pointed by index [(cur_first_msg +
//!   cur_num_msg-1)%max_num_msg] and the first free message ready to be used in
//!   a "send" operation is [cur_first_msg] if circular buffer is extended from
//!   front,
...
//!
//!   This transforms the index in a fixed size priority queue with an embedded
//!   free message queue.
//!
//!-> struct message_t
//!   {
...
//!   the index structure.
template <class VoidPointer>
class mq_hdr_t : public ipcdetail::priority_functor<VoidPointer> {
    typedef VoidPointer void_pointer;
    typedef msg_hdr_t<void_pointer> msg_header;
    typedef typename boost::intrusive::pointer_traits<
        void_pointer>::template rebind_pointer<msg_header>::type msg_hdr_ptr_t;
    typedef typename boost::intrusive::pointer_traits<
        msg_hdr_ptr_t>::difference_type difference_type;
    typedef typename boost::container::container_detail::make_unsigned<
        difference_type>::type size_type;
    typedef typename boost::intrusive::pointer_traits<void_pointer>::
        template rebind_pointer<msg_hdr_ptr_t>::type msg_hdr_ptr_ptr_t;
    typedef ipcdetail::managed_open_or_create_impl<shared_memory_object, 0,
                                                   true, false>
        open_create_impl_t;
   ...
   public:
    ...
    bool is_full() const { return m_cur_num_msg == m_max_num_msg; }

    //! Returns true if the message queue is empty
    bool is_empty() const { return !m_cur_num_msg; }

    //! Frees the top priority message and saves it in the free message list
    void free_top_msg() { --m_cur_num_msg; }
    ...
    msg_header &bottom_msg() { return *mp_index[m_cur_first_msg]; }

    iterator inserted_ptr_begin() const { return &mp_index[m_cur_first_msg]; }

    iterator inserted_ptr_end() const { return &mp_index[this->end_pos()]; }

    iterator lower_bound(const msg_hdr_ptr_t &value,
                         priority_functor<VoidPointer> func) {
        iterator begin(this->inserted_ptr_begin()),
            end(this->inserted_ptr_end());
        if (end < begin) {
            ...
            if (idx_end == ret) {
                ...
                // sanity check, these cases should not call lower_bound
                // (optimized out)
    ...
    msg_header &insert_at(iterator where) {
        ...
        if (where == it_inserted_ptr_beg) {
            ...
            size_type circ_pos = pos >= m_cur_first_msg
                                     ? pos - m_cur_first_msg
                                     : pos + (m_max_num_msg - m_cur_first_msg);
            // Check if it's more efficient to move back or move front
            if (circ_pos < m_cur_num_msg / 2) {
                // The queue can't be full so m_cur_num_msg == 0 or
                // m_cur_num_msg <= pos indicates two step insertion
                if (!pos) {
                    pos = m_max_num_msg;
                ...
                const bool unique_segment =
                    m_cur_first_msg && m_cur_first_msg <= pos;
                const size_type first_segment_beg =
                    unique_segment ? m_cur_first_msg : 1u;
                const size_type first_segment_end = pos;
                const size_type second_segment_beg =
                    unique_segment || !m_cur_first_msg ? m_max_num_msg
                                                       : m_cur_first_msg;
                const size_type second_segment_end = m_max_num_msg;
                const msg_hdr_ptr_t backup = *(
                    &mp_index[0] +
                    (unique_segment ? first_segment_beg : second_segment_beg) -
                    1);
                ...
                *where = backup;
                m_cur_first_msg =
                    m_cur_first_msg ? m_cur_first_msg : m_max_num_msg;
                --m_cur_first_msg;
                ++m_cur_num_msg;
                return **where;
            } else {
                ...
                const size_type first_segment_end =
                    unique_segment ? pos_end : m_max_num_msg - 1;
                const size_type second_segment_beg = 0u;
                const size_type second_segment_end =
                    unique_segment ? 0u : pos_end;
    ...
    //! Returns the inserted message with top priority
    msg_header &top_msg() { return *mp_index[m_cur_num_msg - 1]; }

    //! Returns the inserted message with bottom priority
    msg_header &bottom_msg() { return *mp_index[0]; }

    iterator inserted_ptr_begin() const { return &mp_index[0]; }

    iterator inserted_ptr_end() const { return &mp_index[m_cur_num_msg]; }

    iterator lower_bound(const msg_hdr_ptr_t &value,
                         priority_functor<VoidPointer> func) {
        return std::lower_bound(this->inserted_ptr_begin(),
                                this->inserted_ptr_end(), value, func);
    }
    ...
    msg_header &queue_free_msg(unsigned int priority) {
        ...
        if (m_cur_num_msg && priority > this->bottom_msg().priority) {
            ...
            if (priority > this->top_msg().priority) {
                ...
                it = this->lower_bound(
                    dummy_ptr,
                    static_cast<priority_functor<VoidPointer> &>(*this));
            }
        }
        ...
        const size_type msg_hdr_align =
                        ...
                        index_align =
                            ::boost::container::container_detail::alignment_of<
                                msg_hdr_ptr_t>::value,
                        r_hdr_size =
                            ipcdetail::ct_rounded_size<sizeof(mq_hdr_t),
                                                       index_align>::value,
                        r_index_size = ipcdetail::get_rounded_size<size_type>(
                            max_num_msg * sizeof(msg_hdr_ptr_t), msg_hdr_align),
                        r_max_msg_size = ipcdetail::get_rounded_size<size_type>(
                                             max_msg_size, msg_hdr_align) +
                                         sizeof(msg_header);
    ...
    //! the message index. Never throws.
        ...
        const size_type msg_hdr_align =
                        ...
                        r_hdr_size =
                            ipcdetail::ct_rounded_size<sizeof(mq_hdr_t),
                                                       index_align>::value,
                        r_index_size = ipcdetail::get_rounded_size<size_type>(
                            m_max_num_msg * sizeof(msg_hdr_ptr_t),
                            msg_hdr_align),
                        r_max_msg_size = ipcdetail::get_rounded_size<size_type>(
                                             m_max_msg_size, msg_hdr_align) +
                                         sizeof(msg_header);
   ...
   public:
    ...
    std::size_t get_min_size() const {
        return mq_hdr_t<VoidPointer>::get_mem_size(m_maxmsgsize, m_maxmsg) -
               message_queue_t<VoidPointer>::open_create_impl_t::
...
template <class VoidPointer>
inline typename message_queue_t<VoidPointer>::size_type
message_queue_t<VoidPointer>::get_mem_size(size_type max_msg_size,
                                           size_type max_num_msg) {
...
inline message_queue_t<VoidPointer>::message_queue_t(create_only_t,
    ...
    : m_shmem(create_only, name, get_mem_size(max_msg_size, max_num_msg),
              read_write, static_cast<void *>(0),
...
inline message_queue_t<VoidPointer>::message_queue_t(open_or_create_t,
                                                     ...
                                                     const permissions &perm)
    // Create shared memory and execute functor atomically
    : m_shmem(open_or_create, name, get_mem_size(max_msg_size, max_num_msg),
              read_write, static_cast<void *>(0),
              // Prepare initialization functor
              ipcdetail::msg_queue_initialization_func_t<VoidPointer>(
                  max_num_msg, max_msg_size),
...
inline message_queue_t<VoidPointer>::message_queue_t(open_only_t,
                                                     const char *name)
    // Create shared memory and execute functor atomically
    : m_shmem(open_only, name, read_write, static_cast<void *>(0),
              // Prepare initialization functor
              ipcdetail::msg_queue_initialization_func_t<VoidPointer>()) {}

template <class VoidPointer>
inline void message_queue_t<VoidPointer>::send(const void *buffer,
...
template <class VoidPointer>
inline bool message_queue_t<VoidPointer>::try_send(const void *buffer,
                                                   size_type buffer_size,
                                                   unsigned int priority) {
...
inline bool message_queue_t<VoidPointer>::do_send(
    block_t block, const void *buffer, size_type buffer_size,
    unsigned int priority, const boost::posix_time::ptime &abs_time) {
    ipcdetail::mq_hdr_t<VoidPointer> *p_hdr =
        static_cast<ipcdetail::mq_hdr_t<VoidPointer> *>(
            m_shmem.get_user_address());
    ...
    scoped_lock<interprocess_mutex> lock(
        p_hdr->m_mutex,
        abs_time);  // Inuitive add timed lock (we only use timed_send)
    if (!lock.owns()) {
        return false;
    }
    //---------------------------------------------
    {
        ...
        if (p_hdr->is_full()) {
            BOOST_TRY {
                ...
                switch (block) {
                    ...
                    case timed:
                        do {
                            if (!p_hdr->m_cond_send.timed_wait(lock,
                                                               abs_time)) {
                                if (p_hdr->is_full()) {
#ifdef BOOST_INTERPROCESS_MSG_QUEUE_CIRCULAR_INDEX
                                    --p_hdr->m_blocked_senders;
        ...
        ipcdetail::msg_hdr_t<VoidPointer> &free_msg_hdr =
            p_hdr->queue_free_msg(priority);
    ...
    if (notify_blocked_receivers) {
        p_hdr->m_cond_recv.notify_one(
            abs_time);  // Inuitive add timed notification
    }
#else
...
template <class VoidPointer>
inline void message_queue_t<VoidPointer>::receive(void *buffer,
                                                  size_type buffer_size,
                                                  size_type &recvd_size,
                                                  unsigned int &priority) {
    this->do_receive(blocking, buffer, buffer_size, recvd_size, priority,
                     ptime());
}

template <class VoidPointer>
inline bool message_queue_t<VoidPointer>::try_receive(void *buffer,
                                                      size_type buffer_size,
                                                      size_type &recvd_size,
                                                      unsigned int &priority) {
    return this->do_receive(non_blocking, buffer, buffer_size, recvd_size,
                            priority, ptime());
}

template <class VoidPointer>
inline bool message_queue_t<VoidPointer>::timed_receive(
    void *buffer, size_type buffer_size, size_type &recvd_size,
    unsigned int &priority, const boost::posix_time::ptime &abs_time) {
    if (abs_time == boost::posix_time::pos_infin) {
        this->receive(buffer, buffer_size, recvd_size, priority);
        return true;
    }
    return this->do_receive(timed, buffer, buffer_size, recvd_size, priority,
                            abs_time);
}

template <class VoidPointer>
inline bool message_queue_t<VoidPointer>::do_receive(
    block_t block, void *buffer, size_type buffer_size, size_type &recvd_size,
    unsigned int &priority, const boost::posix_time::ptime &abs_time) {
    ipcdetail::mq_hdr_t<VoidPointer> *p_hdr =
        static_cast<ipcdetail::mq_hdr_t<VoidPointer> *>(
            m_shmem.get_user_address());
        ...
        if (p_hdr->is_empty()) {
            BOOST_TRY {
                ...
                switch (block) {
                    ...
                    case timed:
                        do {
                            if (!p_hdr->m_cond_recv.timed_wait(lock,
                                                               abs_time)) {
                                if (p_hdr->is_empty()) {
#if defined(BOOST_INTERPROCESS_MSG_QUEUE_CIRCULAR_INDEX)
                                    --p_hdr->m_blocked_receivers;
...
template <class VoidPointer>
inline typename message_queue_t<VoidPointer>::size_type
message_queue_t<VoidPointer>::get_max_msg() const {
    ipcdetail::mq_hdr_t<VoidPointer> *p_hdr =
        static_cast<ipcdetail::mq_hdr_t<VoidPointer> *>(
            m_shmem.get_user_address());
...
template <class VoidPointer>
inline typename message_queue_t<VoidPointer>::size_type
message_queue_t<VoidPointer>::get_max_msg_size() const {
    ipcdetail::mq_hdr_t<VoidPointer> *p_hdr =
        static_cast<ipcdetail::mq_hdr_t<VoidPointer> *>(
            m_shmem.get_user_address());
...
template <class VoidPointer>
inline typename message_queue_t<VoidPointer>::size_type
message_queue_t<VoidPointer>::get_num_msg() const {
    ipcdetail::mq_hdr_t<VoidPointer> *p_hdr =
        static_cast<ipcdetail::mq_hdr_t<VoidPointer> *>(
            m_shmem.get_user_address());
    if (p_hdr) {
