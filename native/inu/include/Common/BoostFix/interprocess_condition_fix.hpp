...
// Experimental...
#elif !defined(BOOST_INTERPROCESS_FORCE_GENERIC_EMULATION) && \
...
class interprocess_condition {
   ...
   public:
    //! Constructs a interprocess_condition. On error throws
    ...
    bool notify_one(const boost::posix_time::ptime &abs_time) {
        return m_condition.notify_one(abs_time);
    }  // Inuitive add timed notification

    //! Change the state of all threads waiting on *this to ready.
    //! If there are no waiting threads, notify_all() has no effect.
    bool notify_all(const boost::posix_time::ptime &abs_time) {
        return m_condition.notify_all(abs_time);
    }  // Inuitive add timed notification

    //! Releases the lock on the interprocess_mutex object associated with lock,
    ...
    //! Releases the lock on the interprocess_mutex object associated with lock,
    //! blocks the current thread of execution until readied by a call to
    //! this->notify_one() or this->notify_all(), or until time abs_time is
    //! reached, and then reacquires the lock. Returns: false if time abs_time
