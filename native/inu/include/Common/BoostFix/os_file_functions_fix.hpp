...
//
// Inuitive fix:
// fixed  *get_temporary_path() *names[] variable to use different paths on
// android (line 409)
//
// See http://www.boost.org/libs/interprocess for documentation.
//
...
inline const char *get_temporary_path() { return std::getenv("TMP"); }

inline file_handle_t create_new_file(const char *name, mode_t mode,
...
inline file_handle_t create_or_open_file(
    const char *name, mode_t mode, const permissions &perm = permissions(),
    bool temporary = false) {
    unsigned long attr = temporary ? winapi::file_attribute_temporary : 0;
    return winapi::create_file(
...
inline file_handle_t open_existing_file(const char *name, mode_t mode,
                                        bool temporary = false) {
    unsigned long attr = temporary ? winapi::file_attribute_temporary : 0;
    return winapi::create_file(name, (unsigned int)mode, winapi::open_existing,
                               attr, 0);
}

inline bool delete_file(const char *name) { return winapi::unlink_file(name); }

inline bool truncate_file(file_handle_t hnd, std::size_t size) {
    offset_t filesize;
    if (!winapi::get_file_size(hnd, filesize)) return false;

    typedef boost::make_unsigned<offset_t>::type uoffset_t;
    const uoffset_t max_filesize =
        uoffset_t((std::numeric_limits<offset_t>::max)());
    ...
    if (offset_t(size) > filesize) {
        if (!winapi::set_file_pointer_ex(hnd, filesize, 0,
                                         winapi::file_begin)) {
            return false;
        }
        ...
        for (std::size_t remaining = size - filesize, write_size = 0;
             remaining > 0; remaining -= write_size) {
            const std::size_t DataSize = 512;
            static char data[DataSize];
            write_size = DataSize < remaining ? DataSize : remaining;
            unsigned long written;
            winapi::write_file(hnd, data, (unsigned long)write_size, &written,
                               0);
...
inline bool write_file(file_handle_t hnd, const void *data,
                       std::size_t numdata) {
    unsigned long written;
    return 0 !=
           winapi::write_file(hnd, data, (unsigned long)numdata, &written, 0);
}

inline file_handle_t invalid_file() { return winapi::invalid_handle_value; }
...
inline bool acquire_file_lock(file_handle_t hnd) {
    ...
    return winapi::lock_file_ex(hnd, winapi::lockfile_exclusive_lock, 0, len,
                                len, &overlapped);
}
...
inline bool try_acquire_file_lock(file_handle_t hnd, bool &acquired) {
    ...
    if (!winapi::lock_file_ex(
            hnd,
            winapi::lockfile_exclusive_lock | winapi::lockfile_fail_immediately,
...
inline bool try_acquire_file_lock_sharable(file_handle_t hnd, bool &acquired) {
    ...
    winapi::interprocess_overlapped overlapped;
    std::memset(&overlapped, 0, sizeof(overlapped));
    if (!winapi::lock_file_ex(hnd, winapi::lockfile_fail_immediately, 0, len,
                              len, &overlapped)) {
        return winapi::get_last_error() == winapi::error_lock_violation
               ? acquired = false,
                 true : false;
...
inline bool delete_subdirectories_recursive(
    ...
    if (hFile != winapi::invalid_handle_value) {
        do {
            // If it's not "." or ".." or the pointed root_level
            // dont_delete_this erase it
            if (FileInformation.cFileName[0] != '.' &&
                !(dont_delete_this && count == 0 &&
                  std::strcmp(dont_delete_this, FileInformation.cFileName) ==
                      0)) {
                strFilePath.erase();
                strFilePath =
                    refcstrRootDirectory + "\\" + FileInformation.cFileName;

                // If it's a directory, go recursive
                if (FileInformation.dwFileAttributes &
                    winapi::file_attribute_directory) {
                    // Delete subdirectory
                    if (!delete_subdirectories_recursive(
                            strFilePath, dont_delete_this, count + 1))
                        return false;
                }
                // If it's a file, just delete it
                    ...
                    // winapi::file_attribute_normal) == 0) return
        ...
        // See if the loop has ended with an error or just because we've
        // traversed all the files
        if (winapi::get_last_error() != winapi::error_no_more_files) {
            ...
            if (!bSubdirectory && count) {
                ...
                // FILE_ATTRIBUTE_NORMAL) == 0) return ::GetLastError();
                // Delete directory
...
// pointed by "dont_delete_this"
inline bool delete_subdirectories(const std::string &refcstrRootDirectory,
                                  const char *dont_delete_this) {
    return delete_subdirectories_recursive(refcstrRootDirectory,
                                           dont_delete_this, 0u);
}
...
#elif __ANDROID__
    const char *names[] = {"/storage/emulated/0/Inuitive", "TMPDIR", "TMP",
                           "TEMP"};
#else
...
inline file_handle_t create_new_file(const char *name, mode_t mode,
    ...
    (void)temporary;
    int ret =
        ::open(name, ((int)mode) | O_EXCL | O_CREAT, perm.get_permissions());
    if (ret >= 0) {
...
inline file_handle_t create_or_open_file(
    ...
    // with "O_CREAT only" ::open we don't know if we've created or opened the
    // file.
    while (1) {
...
inline file_handle_t invalid_file() { return -1; }

inline bool close_file(file_handle_t hnd) { return ::close(hnd) == 0; }
...
inline bool try_acquire_file_lock(file_handle_t hnd, bool &acquired) {
    ...
    if (ret == -1) {
        return (errno == EAGAIN || errno == EACCES) ? acquired = false,
                                                      true : false;
    }
    return (acquired = true);
}
...
inline bool delete_subdirectories_recursive(
    ...
    while ((de = ::readdir(d))) {
        ...
        if (dont_delete_this &&
            std::strcmp(dont_delete_this, de->d_name) == 0) {
            continue;
        }
