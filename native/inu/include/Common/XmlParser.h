// Markup.h: interface for the CXmlParser class.
//
...
//#include "OSUtils.h" // memcpy, memset, strcmp...
...
namespace InuCommon 
{
    ...
    #if defined(MARKUP_SAFESTR) // VC++ safe strings
    #define MCD_SSZ(sz) sz,(sizeof(sz)/sizeof(MCD_CHAR))
    #define MCD_PSZCPY(sz,p) COsUtilities::strcpy(MCD_SSZ(sz),p)
    #define MCD_PSZNCPY(sz,p,n) COsUtilities::strncpy(MCD_SSZ(sz),p,n)
    #define MCD_PSZCAT(sz,p) COsUtilities::strcat(MCD_SSZ(sz),p)
    #define MCD_FOPEN(f,n,m) {COsUtilities::fopen(f,n,m);}
    #else // not VC++ safe strings
    #define MCD_SSZ(sz) sz
    ...
    // On windows the file name always be wchar
    #if defined(MARKUP_WCHAR)
    #define MCD_PCSZ_FILENAME const wchar_t*
    #define MCD_T_FILENAME(s) L ## s
    #else
    ...
    struct MCD_CSTR_FILENAME
    {
	    ...
	    MCD_CSTR_FILENAME( const MCD_STR& s ) { pcsz = s.c_str(); };
    ...
    class COMMONUTILITIES_API CXmlParser
    {
    public:
	    CXmlParser();
	    CXmlParser( MCD_CSTR szDoc );
	    CXmlParser( int nFlags );
	    CXmlParser( const CXmlParser& markup );
	    void operator=( const CXmlParser& markup );
	    ~CXmlParser();
