...
class Detection:
    ...
    def to_string(self):
        return "score: {:}, class_id: {:}, xmin: {:}, ymin: {:}, width: {:}, height: {:}".format(
            self.score, self.class_id, self.xmin, self.ymin, self.width, self.height
        )
...
def DecodeBox(raw_boxes, i, anchors, options):
    ...
    if options.num_keypoints:
        for k in range(options.num_keypoints):
            ...
            box_data[4 + k * options.num_values_per_keypoint] = (
                keypoint_x / options.x_scale * anchors[i].w + anchors[i].x_center
            )
            box_data[4 + k * options.num_values_per_keypoint + 1] = (
                keypoint_y / options.y_scale * anchors[i].h + anchors[i].y_center
            )
    return box_data


def ConvertToDetections(
    raw_boxes, anchors_, detection_scores, detection_classes, options
):
    output_detections = []
    for i in range(options.num_boxes):
        ...
        detection = ConvertToDetection(
            box_data[box_offset + 0],
            box_data[box_offset + 1],
            box_data[box_offset + 2],
            box_data[box_offset + 3],
            detection_scores[i],
            detection_classes[i],
...
def ConvertToDetection(
    box_ymin, box_xmin, box_ymax, box_xmax, score, class_id, flip_vertically
):
    # Detection detection;
    ...
    detection = Detection(
        ...
        class_id,
        box_xmin,
        (1.0 - box_ymax if flip_vertically else box_ymin),
        (box_xmax - box_xmin),
        (box_ymax - box_ymin),
    )
...
def ProcessCPU(raw_boxes, raw_scores, anchors_, options):
        ...
        for score_idx in range(options.num_classes):
            ...
            if options.sigmoid_score:
                if options.score_clipping_thresh > 0:
                    score = (
                        -options.score_clipping_thresh
                        if score < -options.score_clipping_thresh
                        else score
                    )
                    score = (
                        options.score_clipping_thresh
    ...
    output_detections = ConvertToDetections(
        raw_boxes, anchors_, detection_scores, detection_classes, options
    )
    return output_detections
...
def main():
    ...
    raw_boxes = np.reshape(
        regressors, int(regressors.shape[0] * regressors.shape[1] * regressors.shape[2])
    )
    raw_scores = np.reshape(
        classificators,
        int(
            classificators.shape[0] * classificators.shape[1] * classificators.shape[2]
        ),
    )
    ...
    print(
        "preprocess cost: {:.2f} ms".format(
            (preprocess_end_time - preprocess_start_time) * 1000
        )
    )
    print(
        "inference cost: {:.2f} ms".format(
            (inference_end_time - inference_start_time) * 1000
        )
    )
    print(
        "postprocess cost: {:.2f} ms".format(
            (postprocess_end_time - postprocess_start_time) * 1000
        )
    )
